...
public class UrlCleanupService {
    ...
    public UrlCleanupService(UserRepository userRepository,
    ...
    public boolean isUserUrlActive(String login, String shortUrl) {
        ...
        LocalDateTime expirationDate = LocalDateTime.parse(urlInfo.getDate(), UtilConstants.FORMATTER)
                .plusMinutes(urlInfo.getExpirationTime());
        return !expirationDate.isBefore(LocalDateTime.now());
    }

    private void deleteUrlFromAllUsers(String shortUrl) {
        userRepository.getUsers().forEach((login, userData) -> {
